...
func (h *RecurringHandler) CreateAction(w http.ResponseWriter, r *http.Request) {
	...
	if itemType != "income" && itemType != "subscription" { itemType = "expense" }
...
func (h *RecurringHandler) DeleteAction(w http.ResponseWriter, r *http.Request) {
	if !requirePost(w, r) { return }
	if id := pathID(r.URL.Path); id != "" { _, _ = h.db.Exec(`UPDATE recurring_items SET is_active = 0 WHERE id = ?`, id) }
	h.redirect(w, r)
}

func (h *RecurringHandler) redirect(w http.ResponseWriter, r *http.Request) { http.Redirect(w, r, "/recurring", http.StatusFound) }
