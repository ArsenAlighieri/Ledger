...
func TestAssetsHandler_CreateAndRemoveItems(t *testing.T) {
	...
	post := func(path string, values url.Values, fn http.HandlerFunc) *httptest.ResponseRecorder {
		...
		if w.Code != http.StatusFound {
			t.Fatalf("%s: expected redirect, got %d: %s", path, w.Code, w.Body.String())
		}
		return w
	}
	...
	var accountID int64
	if err := db.QueryRow(`SELECT id FROM accounts WHERE name = 'Maaş Hesabı'`).Scan(&accountID); err != nil {
		t.Fatalf("account not created: %v", err)
	}
	post("/assets/account/delete/"+stringID(accountID), nil, h.DeleteAccountAction)
	var archived bool
	if err := db.QueryRow(`SELECT is_archived FROM accounts WHERE id = ?`, accountID).Scan(&archived); err != nil || !archived {
		t.Fatalf("account not archived: %v", err)
	}
	...
	var debtID int64
	if err := db.QueryRow(`SELECT id FROM debts WHERE name = 'Kart'`).Scan(&debtID); err != nil {
		t.Fatalf("debt not created: %v", err)
	}
	...
	if err := db.QueryRow(`SELECT id FROM positions LIMIT 1`).Scan(&positionID); err != nil {
